...
// ===== SMART HOME/BACK BUTTON - ADD AT END OF common.js =====
(function(){
    const header = document.querySelector('.top-bar');
    if(!header) return;
    
    var homeIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path><polyline points="9 22 9 12 15 22"></polyline></svg>`;
    ...
    btn.onclick = function(e) {
        e.preventDefault();
        // Agar user pichle page se aaya hai aur wo hamari hi site ka hai to back jao
        if (document.referrer && document.referrer.includes('missiongyanjobs.github.io')) {
            window.history.back();
        } else {
            // Warna Home pe bhejo
    ...
    // Button ka text change karo - Back ya Home
    if (document.referrer && document.referrer.includes('missiongyanjobs.github.io')) {
        ...
        btn.innerHTML = homeIcon + '<span>Home</span>';
    }
    
    var css = `#mgj-home-btn{position:absolute;left:15px;top:50%;transform:translateY(-50%);display:inline-flex;align-items:center;gap:6px;background:#fff;color:#0d47a1!important;padding:8px 14px;border-radius:8px;text-decoration:none!important;font-weight:600;font-size:14px;box-shadow:0 2px 8px rgba(0,0,0,0.15);transition:0.2s;z-index:9999} #mgj-home-btn:hover{background:#0d47a1;color:#fff!important} @media(max-width:768px){#mgj-home-btn{top:12px;transform:none;padding:6px 10px} #mgj-home-btn span{display:none} .top-bar{padding-top:48px!important}}`;
